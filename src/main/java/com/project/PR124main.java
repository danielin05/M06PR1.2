...
public class PR124main {
    ...
    private long trobarPosicioRegistre(RandomAccessFile raf, int registreBuscat) throws IOException {
        ...
        while (raf.getFilePointer() < raf.length()) {
            int registreActual = raf.readInt(); // Llegim el número de registre
            if (registreActual == registreBuscat) {
                return posicio; // Si el registre coincideix, retornem la posició
            }
            posicio += 48; // Cada registre té una mida de 48 bytes
            raf.seek(posicio); // Movem el punter al següent registre
        }
        return -1; // Retornem -1 si no trobem l'estudiant
    }
    ...
    public void llistarEstudiantsFitxer() throws IOException {
        File fitxer = new File(getFilePath());
        ...
        try (RandomAccessFile raf = new RandomAccessFile(fitxer, "r")) {
            raf.seek(0); // Iniciem al principi del fitxer
            while (raf.getFilePointer() < raf.length()) {
                int registre = raf.readInt();
                String nom = llegirNom(raf); // Utilitzem la funció auxiliar per llegir el nom
    ...
    public void afegirEstudiantFitxer(int registre, String nom, float nota) throws IOException {
        ...
        try (RandomAccessFile raf = new RandomAccessFile(fitxer, "rw")) {
            raf.seek(raf.length()); // Ens posicionem al final del fitxer
            raf.writeInt(registre);
            escriureNom(raf, nom); // Funció auxiliar per escriure el nom
            raf.writeFloat(nota);
    ...
    public void consultarNotaFitxer(int registre) throws IOException {
        ...
        try (RandomAccessFile raf = new RandomAccessFile(fitxer, "r")) {
            ...
            if (posicio == -1) {
                ...
                raf.seek(posicio + NAME_POS); // Ens posicionem a la posició del nom
    ...
    public void actualitzarNotaFitxer(int registre, float novaNota) throws IOException {
        ...
        try (RandomAccessFile raf = new RandomAccessFile(fitxer, "rw")) {
            ...
            if (posicio == -1) {
                ...
                raf.seek(posicio + GRADE_POS); // Ens posicionem a la posició de la nota
    ...
    private String llegirNom(RandomAccessFile raf) throws IOException {
        ...
        raf.read(nomBytes); // Llegim els bytes del nom
        return new String(nomBytes, StandardCharsets.UTF_8).trim(); // Convertim i fem trim
    }
    ...
    private void escriureNom(RandomAccessFile raf, String nom) throws IOException {
        ...
        if (nomBytes.length > NAME_MAX_BYTES) {
            System.out.println("Nom massa llarg. Es truncarà.");
            raf.write(nomBytes, 0, NAME_MAX_BYTES); // Escriure només els primers 40 bytes
        } else {
            raf.write(nomBytes); // Escriure el nom
            raf.write(new byte[NAME_MAX_BYTES - nomBytes.length]); // Omplir els espais restants
        }
    }
